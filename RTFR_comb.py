...
while True:
    ...
    for angle in [0, -25, 25]:
        rimg = rotate_image(img_main, angle)
        detected = face.detectMultiScale(rimg, **settings)
        if len(detected):
            detected = [rotate_point(detected[-1], rimg, -angle)]
            break
    ...
    for x, y, w, h in detected[-1:]:
        ...
        #res = Image.fromarray(img_main.astype('uint8'), 'RGB')
        #res = extract_face_rt(res)
